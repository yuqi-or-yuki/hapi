...
export class SyncEngine {
    ...
    async resumeSession(sessionId: string, namespace: string, opts?: { permissionMode?: PermissionMode }): Promise<ResumeSessionResult> {
        ...
        // tiann/hapi upstream resolves the recorded machine strictly (exact
        // machineId, then host match); our fork additionally falls back to
        // any online machine for non-cursor flavors so resume doesn't hard
        // fail just because the originally recorded machine went offline.
        // Cursor stays strict since its chat store data is host-specific.
        // Cursor's chat store is host-specific, and a Pi resume token only means
        // anything on the machine that issued it — both must resolve strictly.
        // Every other flavor keeps the fork's any-online-machine fallback so a
        // resume doesn't hard fail just because the recorded machine went away.
        const strictMachineId = flavor === 'cursor' || (flavor === 'pi' && resumeToken !== undefined)
        const targetMachine = this.resolveOnlineMachineForSession(
            session,
            namespace,
            { strictMachineId }
        ) ?? (strictMachineId ? null : this.machineCache.getOnlineMachinesByNamespace(namespace)[0] ?? null)
        if (!targetMachine) {
            return { type: 'error', message: 'No machine online', code: 'no_machine_online' }
        }
